...
export default function ModuleManagement() {
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        const url = selectedCourse 
            ? `http://localhost:8083/api/courses/${selectedCourse.id}` 
    ...
    const handleDelete = async (id: string) => {
        if (!confirm('Permanently decommission this module?')) return;
        try {
            const res = await fetch(`http://localhost:8083/api/courses/${id}`, { method: 'DELETE' });
        ...
        <DashboardLayout role="super_admin">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '3rem' }}>
                    <div>
                        <h1 style={{ fontSize: '2.5rem', fontWeight: 900, letterSpacing: '-1px' }}>Platform Modules</h1>
                        <p style={{ color: 'var(--text-dim)', fontSize: '1.1rem' }}>Architect the curriculum roadmap and deployment phases.</p>
                    </div>
                    <motion.button onClick={() => handleOpenModal()} className="btn-quantum" style={{ padding: '14px 28px' }}>
                        <Rocket size={18} style={{ marginRight: '8px' }} /> SHIP NEW MODULE
                    </motion.button>
                </div>
                ...
                <div style={{ display: 'flex', gap: '1rem', marginBottom: '2.5rem' }}>
                    <div style={{ flex: 1, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '16px', padding: '0 1rem', display: 'flex', alignItems: 'center', gap: '12px' }}>
                        ...
                        <input type="text" placeholder="Search architectural patterns, modules..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={{ width: '100%', background: 'none', border: 'none', padding: '1.25rem 0', color: 'var(--text-bright)', outline: 'none' }} />
                    </div>
                </div>

                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: '2rem' }}>
                    {loading ? (
                        <p style={{ gridColumn: '1/-1', textAlign: 'center', padding: '4rem', fontWeight: 800 }}>SYNCING MODULE CLUSTER...</p>
                        ...
                        >
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 'auto', borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: '1.5rem' }}>
                                <div>
                                    <div style={{ fontSize: '0.65rem', fontWeight: 800, color: 'var(--text-dim)', textTransform: 'uppercase' }}>Runtime</div>
                                    <div style={{ fontSize: '0.9rem', fontWeight: 900 }}>{c.duration}</div>
                                </div>
                                <div>
                                    <div style={{ fontSize: '0.65rem', fontWeight: 800, color: 'var(--text-dim)', textTransform: 'uppercase' }}>Priority</div>
                                    <div style={{ fontSize: '0.9rem', fontWeight: 900, color: '#10b981' }}>ALPHA_9</div>
                                </div>
                            </div>
                        </motion.div>
                    ))}
                ...
                {isModalOpen && (
                    <div style={{ position: 'fixed', inset: 0, zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(12px)' }}>
                        <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="glass-panel" style={{ width: '90%', maxWidth: '600px', padding: '3rem', borderRadius: '40px' }}>
                            <h2 style={{ fontSize: '2rem', fontWeight: 900, marginBottom: '2rem' }}>Deploy Module Configuration</h2>
                            <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                    <label style={{ fontSize: '0.75rem', fontWeight: 800, color: 'var(--text-dim)' }}>MODULE TITLES</label>
                                    <input value={formData.title} onChange={(e) => setFormData({...formData, title: e.target.value})} placeholder="e.g. Quantum Microservices" style={inputStyle} />
                                </div>
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                    <label style={{ fontSize: '0.75rem', fontWeight: 800, color: 'var(--text-dim)' }}>ARCHITECTURAL OVERVIEW</label>
                                    <textarea value={formData.description} onChange={(e) => setFormData({...formData, description: e.target.value})} placeholder="Describe the deployment roadmap..." style={{ ...inputStyle, minHeight: '120px', resize: 'none' }} />
                                </div>
                                <div style={{ display: 'flex', gap: '1.5rem' }}>
                                    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                        <label style={{ fontSize: '0.75rem', fontWeight: 800, color: 'var(--text-dim)' }}>DURATION (WKS/HRS)</label>
                                        <input value={formData.duration} onChange={(e) => setFormData({...formData, duration: e.target.value})} placeholder="12 Weeks" style={inputStyle} />
                                    </div>
                                    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                        <label style={{ fontSize: '0.75rem', fontWeight: 800, color: 'var(--text-dim)' }}>RESOURCE COST</label>
                                ...
                                <div style={{ display: 'flex', gap: '1rem', marginTop: '2rem' }}>
                                    ...
                                    <button type="submit" className="btn-quantum" style={{ flex: 2, padding: '14px', borderRadius: '16px', fontWeight: 800 }}>{selectedCourse ? 'RE-MAP ARCHITECTURE' : 'INITIATE DEPLOYMENT'}</button>
                                </div>
                            </form>
                        </motion.div>
                    </div>
