...
import {
    ...
    Download, Upload, Layers, X, UserPlus, Info, ArrowRight,
...
export default function TrainerUnifiedConsole() {
    ...
    // Permission state (Mocking full access for Trainer/Admin)
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const myBatchNames = new Set(myBatches.map((b: Batch) => b.batchName));

                setBatches(myBatches);
                setSessions((sessionsRes.data || []).filter((s: any) => myBatchNames.has(s.batchName)));
    ...
    const stats = useMemo(() => [
        { label: "Total Programs", value: courseGroups.length.toString(), icon: Target, color: "#8b5cf6" },
        { label: "Assigned Batches", value: batches.length.toString(), icon: Users, color: "#3b82f6" },
        { label: "Live Rooms", value: sessions.filter(s => s.status === 'LIVE' || s.status === 'UPCOMING').length.toString(), icon: Video, color: "#10b981" },
        { label: "Total Students", value: batches.reduce((acc, b) => acc + (b.totalStudents || 0), 0).toString(), icon: GraduationCap, color: "#f59e0b" },
    ], [courseGroups, batches, sessions]);
            ...
            <AnimatePresence mode="wait">
                {activeTab === 'catalog' && (
                    <motion.div key="catalog" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                        ...
                        {!selectedCourseId && !selectedBatch ? (
                            /* ── IMAGE 1: COURSE GRID (Faculty View) ── */
                                    ...
                                    >
                                        ...
                                        <div className="p-8 flex-1 flex flex-col">
                                            ...
                                            <div className="mt-10 pt-6 border-t border-white/5 flex items-center justify-between">
                                                <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Manage Syllabus</span>
                            ...
                            /* ── IMAGE 2: BATCH SELECTION (Faculty View) ── */
                            <div className="space-y-8">
                                <div className="flex justify-between items-end">
                                    ...
                                    <p className="text-slate-500 font-bold uppercase text-xs tracking-widest mt-2">Active Assigned Cohorts</p>
                                </div>
                            ...
                            /* ── IMAGE 3 & 4: BATCH CONTROL PANEL (Faculty View) ── */
                            <motion.div key="batch-control" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-10 pb-20">
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                                    ...
                                    <div className="bg-[#0b0b1a] border border-white/5 rounded-[2.5rem] overflow-hidden shadow-2xl">
                                        <div className="p-8 pb-4 flex justify-between items-center">
                                            <h3 className="text-xl font-black text-white uppercase tracking-tight flex items-center gap-3 font-[Rajdhani]">
                                                <Users className="text-blue-400" /> Enrollment Roster ({selectedBatch.totalStudents})
                                            </h3>
                                        </div>
                                        <div className="p-8 pt-0 space-y-4 max-h-[400px] overflow-y-auto custom-scrollbar">
                                            {allStudents.filter(s => selectedBatch.studentIds?.includes(s.id)).length > 0 ? (
                                                ...
                                                <div className="py-20 text-center font-bold text-slate-700 uppercase tracking-widest text-sm border-2 border-dashed border-white/5 rounded-3xl">No students assigned to this cohort</div>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-[#0b0b1a] border border-white/5 rounded-[2.5rem] overflow-hidden shadow-2xl">
                                        <div className="p-8 pb-4 flex justify-between items-center">
                                            <h3 className="text-xl font-black text-white uppercase tracking-tight flex items-center gap-3 font-[Rajdhani]">
                                                <Video className="text-violet-400" /> Live Rooms
                                            </h3>
                                        ...
                                        <div className="p-8 pt-0 space-y-4 max-h-[400px] overflow-y-auto custom-scrollbar">
                                            {sessions.filter(s => s.batchName === selectedBatch.batchName).length > 0 ? (
                                                ...
                                                <div className="py-20 text-center font-bold text-slate-700 uppercase tracking-widest text-sm border-2 border-dashed border-white/5 rounded-3xl">No sessions active for this batch</div>
                                            )}
                                        </div>
                                    </div>
                                </div>
                ...
                {/* Other tabs as placeholders or simple views */}
                {activeTab === 'sessions' && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {sessions.map(s => (
                    ...
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                        ...
                        <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative w-full max-w-md bg-[#0d0d1f] border border-white/10 rounded-[2.5rem] p-10 overflow-hidden shadow-2xl">
                            ...
                            <h3 className="text-2xl font-black text-white uppercase tracking-tight font-[Rajdhani] mb-6 flex items-center gap-3">
                                <FolderPlus className="text-emerald-400" /> Create Folder
                            </h3>
                            <div className="space-y-6">
                                <div>
                                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 block">Folder Name</label>
                                    <input
                                        type="text"
                                        value={newFolderName}
                                ...
                                <div className="flex gap-4">
                                    ...
                                    <button
                                        ...
                                        {uploading ? 'Initializing...' : 'Initialize'}
                                    </button>
                                </div>
                            </div>
                        </motion.div>
                        ...
                        <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative w-full max-w-md bg-[#0d0d1f] border border-white/10 rounded-[2.5rem] p-10 overflow-hidden shadow-2xl text-center">
                            ...
                            <div className="flex gap-4">
                                ...
                                <button
                                    ...
                                    {uploading ? 'Pushing...' : 'Push to Cloud'}
                                </button>
                            </div>
                        </motion.div>
                    </div>
