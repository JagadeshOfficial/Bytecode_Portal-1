    ...
    return (
        ...
        >
            <AnimatePresence mode="wait">
                                    ...
                                    <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-10 relative z-10 px-2">
                                        <div className="flex flex-col gap-1">
                                            <h3 className="text-xl font-black text-white uppercase tracking-[0.05em] font-[Rajdhani] flex items-center gap-3">
                                                ...
                                                Curriculum Asset Hub
                                            </h3>
                                            <div className="flex items-center gap-2 text-[8px] font-black uppercase tracking-[0.2em] text-slate-500 ml-14">
                                                <button onClick={() => setCurrentFolderId(null)} className={`hover:text-blue-400 transition-colors ${!currentFolderId ? 'text-blue-400 font-bold' : ''}`}>Root</button>
                                                {currentFolderId && (
                                                    <>
                                                        <ChevronRight size={8} />
                                                        <span className="text-blue-400 font-bold">{materials.find(m => m.id === currentFolderId)?.name || 'Folder'}</span>
                                                    </>
                                                )}
                                            </div>
                                        </div>
                                        <div className="flex flex-wrap justify-end gap-2">
                                            <button
                                                ...
                                                className="px-4 py-2 bg-[#1a1a2e] border border-white/5 text-slate-400 rounded-lg text-[10px] font-black uppercase tracking-widest flex items-center gap-2 hover:border-blue-500/30 hover:text-blue-400 transition-all">
                                                <FolderPlus size={14} /> Organization
                                            </button>
                                            <button
                                                onClick={() => setShowUploadModal(true)}
                                                className="px-4 py-2 bg-blue-600 border border-blue-500 text-white rounded-lg text-[10px] font-black uppercase tracking-widest flex items-center gap-2 hover:bg-blue-500 transition-all shadow-lg shadow-blue-500/20">
                                                    ...
                                                    <motion.div
                                                        ...
                                                        className={`relative overflow-hidden group p-4 rounded-xl border transition-all duration-300 cursor-pointer flex items-center gap-4
                                                            ${isFolder
                                                                ...
                                                                : item.type === 'VIDEO'
                                                                    ? 'bg-blue-500/5 border-blue-500/10 hover:border-blue-500/40'
                                                                    : 'bg-rose-500/5 border-rose-500/10 hover:border-rose-500/40'
                                                            }`}
                                                    >
                                                        <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-all duration-500 
                                                            ${isFolder ? 'bg-amber-500/10 text-amber-500' :
                                                                item.type === 'VIDEO' ? 'bg-blue-500/10 text-blue-500' : 'bg-rose-500/10 text-rose-500'}`}>
                ...
                {activeTab === 'batches' && (
                    <motion.div key="batches" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
                        <div className="flex justify-between items-center bg-white/5 border border-white/5 p-8 rounded-[2rem] backdrop-blur-xl">
                            <div>
                                <h3 className="text-2xl font-black text-white uppercase tracking-tight font-[Rajdhani]">Active Assigned Batches</h3>
                                <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mt-1">Manage all your cohorts from a single pane</p>
                            </div>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            {batches.map((batch) => (
                ...
                {
                    activeTab === 'sessions' && (
                        <motion.div key="sessions-tab" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center py-32 text-slate-700 border-2 border-dashed border-white/5 rounded-[3rem]">
                            <Video size={80} className="mb-6 opacity-10 animate-pulse text-violet-400" />
                            <h4 className="text-xl font-black text-white uppercase tracking-[0.2em] font-[Rajdhani]">Live Stream Hub</h4>
                            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-2">Connecting to Bytecode RTC Engine...</p>
                        </motion.div>
                    )
                }

                {
                    activeTab === 'recordings' && (
                        <motion.div key="recordings-tab" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center py-32 text-slate-700 border-2 border-dashed border-white/5 rounded-[3rem]">
                            <PlayCircle size={80} className="mb-6 opacity-10 animate-pulse text-rose-400" />
                            <h4 className="text-xl font-black text-white uppercase tracking-[0.2em] font-[Rajdhani]">Recording Archives</h4>
