...
REQUIRED_PORTS=(8761 8080 8081 8082 8083 8084 8085 8086 8087 8088 3000)
...
check_mongodb() {
  if ! (echo > /dev/tcp/127.0.0.1/27017) >/dev/null 2>&1; then
    echo "Error: MongoDB is not listening on localhost:27017."
    exit 1
  fi
}
...
#  check_mongodb
  ...
  for service in "${SERVICES[@]}"; do
    ...
    if [[ ! -x "$service_dir/mvnw" ]]; then
      echo "Error: missing executable Maven wrapper at $service_dir/mvnw."
      exit 1
    fi
  done
...
check_existing_runtime_state() {
  local service
  for service in "${SERVICES[@]}" "$FRONTEND_NAME"; do
...
print_summary() {
  cat <<EOF
--------------------------------------------------
Stack launched in background processes.
Logs:      $LOG_DIR
PID files: $PID_DIR
...
main() {
  echo "Starting Bytecode Trainings stack..."
  ensure_runtime_dirs
  verify_prerequisites
  check_existing_runtime_state
